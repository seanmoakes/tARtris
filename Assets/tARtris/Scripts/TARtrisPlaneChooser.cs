...
namespace Mongoose
{
    public class TARtrisPlaneChooser : ARBase
    {
        [SerializeField]
        private GameObject m_PlanePrefab;

		[SerializeField]
		private string  tARtrisSceneStr;
		...
		void OnGUI()
		{
			if (!doneChoosingPlane) {
				GUI.Box (new Rect (Screen.width / 4, 10, Screen.width / 2, 100), "Select the surface you wish to play tARtris on");
			}
		}
        ...
        private void Update()
        {
			...
			if (doneChoosingPlane) {
				//before loading tARtris scene, get rid of the planes you created
